...
final readonly class RelationshipsContentDescriptor
{
     ...
     * following JSON:API include patterns for eager loading related resources.
     *
     * ```php
     * $descriptor = RelationshipsContentDescriptor::create([
     *     'users' => ['profile', 'posts', 'comments'],
     *     'posts' => ['author', 'comments', 'tags'],
     * ]);
     * // Client can request: ?relationships[users]=profile,posts
     * ```
     *
     * @param  array<string, array<int, string>>                                                                                                                                                $relationships Associative array where keys are resource type names and values are arrays of available relationship names for that resource. Used to generate enum constraints that restrict relationship inclusion to explicitly allowed associations.
     * @return null|array{name: string, description: string, schema: array{type: string, properties: array<string, array{type: string, items: array{type: string, enum: array<int, string>}}>}} Returns null if no relationships are provided, otherwise returns an OpenRPC content descriptor array with name, description, and schema properties defining the relationship parameter structure
     */
